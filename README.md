...
데일리 HTML은 static-first로 동작하지만, `ACTIVIST_PUBLIC_API_URL`이 있으면 브라우저에서 `reports`, `latest_snapshot`, `articles` read API를 호출해 다른 일자 목록, `DB 이슈 흐름`, DB 기사 검색, 기사별 `DB 맥락 보기`를 보강합니다. `DB 맥락 보기`는 같은 story 또는 제목 토큰 기반 관련 기사를 불러와 매체 확산과 최근 흐름을 보여줍니다. API가 실패해도 정적 기사 페이지는 그대로 표시됩니다.
