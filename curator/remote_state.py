...
def mysql_runtime_enabled() -> bool:
    return os.environ.get("CURATOR_DATA_SOURCE", "").strip().casefold() in {"mysql", "remote", "api"}


def _required() -> bool:
    return os.environ.get("CURATOR_REQUIRE_REMOTE_API", "").strip().casefold() in {"1", "true", "yes", "on"} or mysql_runtime_enabled()
...
def _merge_runtime_source_rights(
    ...
    for row in source_right_rows:
        if str(row.get("status") or "") not in {"active", "pending", "expired", "revoked"}:
            continue
        rights_records.append(
            {
                ...
                or (f"sha256:{row.get('evidence_hash')}" if row.get("evidence_hash") else ""),
...
def _derived_record_is_authorized(
    ...
    *,
    ...
    normalized_right_ids = [str(right_id) for right_id in right_ids if right_id] if isinstance(right_ids, list) else []
...
def fetch_runtime_resource(
    ...
    *,
    ...
    if not remote_api_configured():
        if _required():
            raise RuntimeError("MySQL runtime requested but ACTIVIST_API_URL/SECRET are missing")
        return []
    ...
    while len(records) < max_records:
        response = post_remote_action(
            ...
            {
                "resource": resource,
                "limit": min(100, max_records - len(records)),
                "after": cursor,
        ...
        if not response.get("ok"):
            raise RuntimeError(f"runtime state export failed for {resource}: {response.get('error') or 'unknown_error'}")
...
def _cluster(row: dict[str, object], articles: list[dict[str, object]]) -> dict[str, object]:
    payload = _json_value(row.get("payload_json"), {})
    cluster = dict(payload) if isinstance(payload, dict) else {}
    cluster.update({key: value for key, value in row.items() if key != "payload_json" and value is not None})
    ...
    cluster.setdefault("last_article_at", cluster.get("last_article_seen_at") or cluster.get("published_at"))
    cluster.setdefault("last_article_seen_at", cluster.get("last_article_at"))
    cluster.setdefault("first_seen_at", cluster.get("published_at") or cluster.get("last_article_at"))
    if articles:
        ...
        synthetic = _article(
            {
                ...
                "published_at": cluster.get("published_at") or cluster.get("last_article_seen_at"),
                "source": "",
...
def hydrate_runtime_state(
    ...
    retention_days = int(retention.get("retention_days", 60)) if isinstance(retention, dict) else 60
    max_articles = int(retention.get("max_articles", 5000)) if isinstance(retention, dict) else 5000
    max_telegram = int(retention.get("max_telegram_records", 5000)) if isinstance(retention, dict) else 5000
    since = now - timedelta(days=max(2, retention_days))

    article_rows = fetch_runtime_resource("articles", since=since, max_records=max_articles)
    story_rows = fetch_runtime_resource("stories", since=since, max_records=1000)
    all_time = datetime(1970, 1, 1, tzinfo=timezone.utc)
    channel_rows = fetch_runtime_resource("telegram_channels", since=all_time, max_records=1000)
    message_rows = fetch_runtime_resource("telegram_messages", since=since, max_records=max_telegram)
    match_rows = fetch_runtime_resource("telegram_article_matches", since=since, max_records=max_telegram * 2)
    signal_rows = fetch_runtime_resource("telegram_issue_signals", since=since, max_records=3000)
    outbox_rows = fetch_runtime_resource("delivery_outbox", since=since, max_records=3000)
    source_right_rows = fetch_runtime_resource("source_rights", since=all_time, max_records=3000)
    collection_runs = fetch_runtime_resource("collection_runs", since=since, max_records=3000)
    company_rows = fetch_runtime_resource("companies", since=since, max_records=5000)
    governance_rows = fetch_runtime_resource("governance_events", since=since, max_records=3000)
    ...
    state["published_clusters"] = [cluster for cluster in clusters if str(cluster.get("status") or "") == "published"]
    state["pending_clusters"] = [cluster for cluster in clusters if str(cluster.get("status") or "") != "published"]
    state["telegram_source_channels"] = [dict(row) for row in channel_rows]
    ...
    for row in signal_rows:
        ...
        signal.update({key: value for key, value in row.items() if key != "payload_json" and value is not None})
    ...
    for row in outbox_rows:
        ...
        if isinstance(payload, dict):
            ...
            delivery["payload_text"] = payload.get("text") or payload.get("payload_text") or ""
