...
복구 실행의 메시지 없는 페이지 완료 checkpoint도 hydrated `issue_signals`를 재전송하지 않는다. 중간 요청은 채널 metadata와 durable cursor만 전송하고 채널 건수와 signal 0건 ACK를 정확히 확인하며, signal은 전체 복구 창을 마지막으로 재계산한 뒤의 metadata/rebuild 요청에서만 전송한다. 메시지 batch는 POST 전에 각 메시지 채널 identity가 정확히 하나의 채널 snapshot에 대응하는지 확인하고, 응답의 메시지·매치·채널 건수가 모두 정확히 일치한 뒤에만 cursor를 전진시킨다.
