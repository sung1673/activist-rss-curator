...
`curator.release_gate`는 실제 시간을 대신하거나 지표를 생성하지 않는다. 운영 DB·관측 시스템에서 내보낸 14일 shadow 결과와 7일 연속 운영·성능 지표, 사람 라벨 benchmark 보고서를 읽어 공개 전환 가능 여부를 판정한다.

> 현재 상태(2026-07-16): 판정 consumer와 fail-closed 검사는 구현됐지만, 아래 네 파일을 실제 운영 원시 데이터에서 만드는 production exporter는 아직 없다. 따라서 유효한 `evidence_run_id`를 만들 수 없으며 전환 게이트는 운영상 비활성 상태다. 합성값이나 수동 상수로 artifact를 만드는 workflow를 추가하지 않는다. 수집 run의 코드 revision, legacy/candidate 사건 snapshot과 차이 검수, outbox·incident 원시 이력, 공식 근거 분모/분자, 실제 모바일 RUM, 보호된 사람 라벨 원본이 저장된 뒤 producer를 연결한다.

## 증빙 묶음

한 번의 판정에는 다음 네 파일이 필요하다.
...
- `official_ingest_success_rate` 0.99 이상
- `official_lag_p95_minutes` 45 이하
- `delivery_success_rate` 0.995 이상
- `delivery_failure_detection_p95_minutes` 10 이하
...
`performance.jsonl`의 각 행은 가용성 0.999 이상, 모바일 LCP p75 2.5초 이하, INP p75 200ms 이하, CLS p75 0.1 이하를 충족해야 한다. 평균이나 7일 합산 값이 좋은 것으로 실패한 하루를 상쇄할 수 없으며 일별 모든 게이트를 통과해야 한다.
...
보고서에는 라벨 출처, 입력 데이터셋 SHA-256, 환경, 코드 리비전이 포함된다. 라벨 출처가 `human|adjudicated` 이외이거나 500 pair·300 사건, same-story precision 0.97, relevance recall 0.95 중 하나라도 미달하면 최종 전환 게이트를 통과할 수 없다. 작은 합성 fixture는 낮춘 임계값으로 benchmark 함수를 호출하더라도 `is_synthetic=true`, `release_eligible=false`로 기록되어 최종 판정에서 거부된다.
...
향후 운영 export 작업은 네 파일을 `governance-release-evidence` artifact로 업로드한다. producer가 구현된 뒤 `Governance release transition gate`를 수동 실행하면서 해당 artifact를 만든 workflow run ID와 artifact 이름을 입력한다. 워크플로는 `github.sha`를 `--expected-revision`으로 전달해 선택한 branch/tag의 checkout과 증빙이 같은 revision인지 확인한다. 원본 artifact를 수정하지 않고 보고서 두 개를 90일 보존하며, 판정이 `0`이 아니면 최종 job을 실패시킨다.

이 워크플로는 전환을 자동 수행하지 않는다. 통과 보고서, 편집 검수 이력, 법률 검토, 사용자 평가를 확인한 사람이 별도 승인한 뒤 공개 UI를 전환한다.
