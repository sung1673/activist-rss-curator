...
OpenDART SourceRight 사전조건이 붙은 guarded write는 요청 본문 한도를 넘지 않도록
문서 최대 40건씩 전송한다. 각 문서 배치에는 그 문서를 참조하는 사건과 해당
회사만 함께 넣는다. 일반/KIND 쓰기의 기존 1,800건 배치와 회사 마스터 전용
배치는 바꾸지 않는다. 전송 전 각 DART 사건이 payload 안의 문서 정확히 1건만
참조하는지 검사하므로 여러 배치에 중복되거나 문서 없는 사건이 누락될 수 없다.
각 배치는 재시도 없이 정확히 한 번 전송하고, 서버의
회사·문서·사건·권한·run ACK가 제출 건수와 모두 같을 때만 ACK 수를 올린다.
인증·backend binding·DART SourceRight·guard·release/deployment 사전조건의
결정적 오류가 발생하면 다음 배치와 final run 쓰기를 즉시 중단한다.

실패한 배치와 최종 run 쓰기는 체크포인트와 collection run에 제한된 원격
텔레메트리만 남긴다. 허용 필드는 배치 종류·순번, HTTP 상태, allowlist로
검증한 안정 오류 코드, 응답 본문 바이트 수, 경과 밀리초, 예외 클래스명과
검증된 SQLSTATE·숫자 driver code·검증 사유다. 응답 본문, 예외 메시지, URL,
토큰·서명·nonce, 회사·문서·사건·run 식별자는 저장하지 않는다. 알 수 없거나
길이 초과·비 ASCII 오류 문자열은 `remote_error_unclassified`로 치환한다.
중간 배치가 실패하면 이후 final run에 실패 집계를 기록할 수는 있지만 해당
날짜 창은 `completed_windows`에 절대 넣지 않는다.
