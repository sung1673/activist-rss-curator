...
DART를 호출하는 모든 workflow는 실제 HTTP 요청 직전에 ops 인증의 `POST /api/v1/ops/dart-quota`로 물리 요청 1건을 소비한다. `action=consume` 요청에는 고유 `attempt_id`, 현재 KST `quota_day`, 비밀이 아닌 소문자 SHA-256 `credential_id`, `operation=list|corp_code`, 실행 `code_revision`, 검증된 backend binding을 포함한다. 모든 키를 합산한 KST 일일 상한은 40,000건이며 각 실행의 별도 안전 상한은 10,000건이다. 서버의 HTTP 200은 transaction commit 성공 후 기존 transaction과 다른 fresh PDO connection이 동일 backend binding에서 exact attempt request hash·quota day·credential 상태를 readback한 경우에만 반환된다. 클라이언트는 첫 ACK를 검증한 뒤 필드와 값이 완전히 같은 POST를 별도로 replay하고 두 번째 ACK의 `duplicate=true`를 확인해야 한다. 내부 transport retry 때문에 첫 ACK가 이미 duplicate여도 이 명시적 replay는 생략하지 않으며, 한 논리 consume의 `used`는 1만 증가한다. 같은 요청 재전송은 소비량을 늘리지 않으며 다른 payload로 같은 ID를 쓰면 409다. commit 또는 독립 readback을 증명하지 못하면 503 `dart_quota_persistence_failed`와 `transaction_commit_failed|transaction_state_invalid|transaction_readback_connection_failed|transaction_readback_binding_failed|transaction_readback_attempt_failed|transaction_readback_day_failed|transaction_readback_credential_failed` 중 하나만 반환한다. OpenDART 코드 `020`을 받으면 같은 attempt로 `action=block_020`, `reason=opendart_status_020`을 기록하고 그 exact POST의 duplicate replay까지 확인한 뒤 해당 키만 다음 KST 자정까지 차단해 다른 유효 키로 계속한다. 코드 `901`도 `action=disable_901`, `reason=opendart_status_901`의 duplicate replay를 확인한 뒤 해당 키만 durable disable한다. `GET /ops/dart-quota?quota_day=YYYY-MM-DD`는 전역 사용량과 credential별 상태를 반환하는 읽기 전용 조회다. 키 원문은 API, DB, 로그, checkpoint, artifact에 기록하지 않는다.
...
guarded DART snapshot은 native PDO의 buffered-query 기본값에 의존하지 않는다.
단일 행·단일 열·복수 행 조회 모두 다음 SQL이나 rollback 전에
`closeCursor()`를 확인한다. CI는 `PDO::MYSQL_ATTR_USE_BUFFERED_QUERY=false`인
PHP 7.3/MySQL 8 환경에서 신규 적재, 동일 payload replay, 정정 lineage,
actor 관계, 검증 실패 rollback을 실행하며 MySQL driver 2014가 없는지 검사한다.
