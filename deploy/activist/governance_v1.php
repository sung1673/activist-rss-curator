...
function v1_runtime_resource(array $config, string $resource): ?array {
    $resources = array(
        ...
        'telegram_channels' => array('handle', 'SELECT handle, telegram_channel_id, title, description, joined, enabled, source, source_type, is_public_channel, quality_score, last_message_id, last_collected_at, last_error, updated_at FROM ' . table_name($config, 'telegram_channels'), 'updated_at'),
